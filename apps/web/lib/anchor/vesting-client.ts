 ...
 *
 * StreamAccount layout — 228 bytes total (after 8-byte discriminator):
 *   [8..40]   creator              Pubkey
 ...
 *   [188..220]name                 [u8; 32]
 *
 * create_stream instruction — 80 bytes data, 9 accounts:
...
export const STREAM_ACCOUNT_SIZE = 228;
...
// ─── Decode raw 228-byte StreamAccount ───────────────────────────────────────
...
/**
 * build create_stream: 80 bytes, 9 accounts.
 * Args: total_amount(u64) start_time(i64) end_time(i64) cliff_time(i64) seed(u64) name([u8;32])
...
function mkCreateIx(
  ...
  const data = Buffer.alloc(80); // 8 disc + 5×8 args + 32 name
