    ...
    if (csvMode && csvRows.length > 0) {
      ...
      for (let i = 0; i < csvRows.length; i++) {
        ...
        const ok = await submit({
          ...
          name: streamName ? `${streamName} #${i + 1}` : undefined,
      ...
      <Section title="General Details">
        ...
        <div>
          ...
          <SInput value={streamName}
            ...
            placeholder="e.g. Cliff vesting Q4 2026"
            mono={false} />
          <div style={{ fontSize: 11, color: C.muted, marginTop: 5 }}>
            {csvMode
              ? `In batch mode, each row is suffixed " #1", " #2", etc. (${streamName.length}/31 base chars)`
              : streamName.length > 0
