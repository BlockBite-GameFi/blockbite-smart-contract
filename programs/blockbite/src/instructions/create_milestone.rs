...
use crate::constants::{MIN_LEVEL, MAX_LEVEL, DIFFICULTY_EASY, DIFFICULTY_HARD};
...
/// Validates `token_amount`, checks the campaign budget, validates level and
/// difficulty, populates the MilestoneAccount, and updates the campaign's
/// allocated amount and milestone count.
///
/// Game-verification is free — no protocol fee is charged on `create_milestone`.
...
pub fn init_milestone(
    ...
    require!(
        new_allocated <= campaign.total_budget,
